...
if (global.fetch) {
  ...
  global.fetch = function (url, options) {
    if (globalProxy) {
      if (url instanceof global.Request) {
        console.warn('[BIFROST]fetch(request) not support, instead of fetch(url,options)');
      } else {
        options = options || {};
        ...
        // 设置代理地址
        uri.protocol = 'http:';
        uri.host = globalProxy.host;
        uri.port = globalProxy.port;

        // 添加原始地址信息
        options.headers['x-whistle-real-host'] = port ? `${host}:${port}` : host;
        if (isHttps) options.headers['x-whistle-https-request'] = 1;
